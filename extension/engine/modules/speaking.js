 ...
 * (녹음 단계는 고급 설정의 '스피킹 녹음 단계' 를 켜야 진행한다. 기본은 꺼져 있다.)
 *
 * **아직 남은 문제 — 크롬 확장에서만.**
 * 마이크 단계가 도중에 조용히 서는 일이 있다. 오래 '마지막 카드에서 멈춘다'고 적어 두었는데
 * 확인해 보니 틀린 말이었다: 카드를 5장으로 늘리면 **두 번째 카드에서** 선다.
 * 기준은 카드가 아니라 시간이고, 언제나 **스피킹을 시작한 지 30초쯤**이다.
 *
 * 원인은 이 파일이 아니라 크롬이다. MV3 의 배경 스크립트(서비스 워커)는 크롬 쪽 이벤트가
 * 30초쯤 없으면 잠재워 버리고, 그러면 기다리던 타이머까지 같이 멈춰 로그도 오류도 없이 선다.
 * 스피킹은 열두 모드 중 **페이지를 옮겨 다니지 않는 유일한 모드**라 그 30초 동안 이벤트가
 * 하나도 생기지 않는다(다른 모드는 학습을 시작할 때마다 화면이 넘어가며 탭 이벤트가 생긴다).
 *
 * 확인한 것: 카드 수·녹음 시간·getUserMedia 는 상관이 없다. 확장 페이지가 한 번 열리면
 * 워커가 깨어나고 **자동화는 멈췄던 자리에서 그대로 이어진다**(낭독 3→5장, 이어서 쉐도잉).
 * 효과가 없던 것: 0.5분 알람, 학습 페이지에서 포트로 보내는 신호, 같은 곳에서 보내는 보통 메시지.
 * 그래서 지금은 '깨어 있게 두기'(content/keepalive.js) 와 함께
 * **꺼져도 저장된 위치에서 스스로 이어하기**(background.js 의 recoverInterruptedRun) 를 둔다.
 * PC 앱과 안드로이드는 서비스 워커가 없어 이 문제가 없다.
 */
