...
* **문법** — 문법훈련(GClass) 자동 풀이.
  ...
  **배열형 빈칸**(빈칸 여러 개 + 화면의 힌트 단어, `6`) · 분류 · 짝맞추기.
  정답은 이 순서로 찾습니다 —
  ① 화면에 들어 있는 정답(`.gclass-q-answer-text`, 숨겨져 있어도 읽음)
  ② 단어장 ③ **페이지가 들고 있는 정답 데이터**(채점을 브라우저가 하므로 정답이
  전역 변수 어딘가에 있습니다. 실행 중에 그것을 찾아 씁니다)
  ④ 개념 톡은 **바로 다음 설명 카드가 정답을 풀어서 말해 주므로** 그 문장과 대조
  ⑤ 그래도 모르면 찍은 뒤 **채점 결과를 기억**해 오답을 지워 나갑니다.
