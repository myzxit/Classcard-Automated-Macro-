...
* **문법** — 문법훈련(객관식) 자동 풀이. 단어장에서 답을 찾으면 그 보기를 고르고,
  **답을 모르면 보기를 하나씩 확인하며 오답을 기억**해 다음 시도에서 제외합니다
  (보기가 4개면 최대 4번 안에 정답). 정답표가 없어도 풀립니다
* **단어장 가져오기** — 현재 학습 페이지에서 단어/뜻 데이터를 추출해 계정별로 보관
* **전체 자동화** — 단어장 목록에서 맨 아래 set 부터 위로 올라가며 단어/문장 자동 판별 →
  학습구간을 '전체 카드 학습'으로 변경 → 단어장 갱신 →
  암기 → 리콜 → 스펠 → 매칭/스크램블 → (문법이 있는 set 이면)문법 → 테스트를 차례로 수행.
