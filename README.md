...
* **스피킹** — 세트의 **스피킹**(`/Paragraph`) 화면 자동화.
  ...
  **낭독 · 쉐도잉 · 녹음**은 녹음한 목소리가 그대로 선생님께 올라가고 자동 채점되는 단계라,
  ...
  (크롬 확장에서만, 시작하고 30초쯤 지나면 조용히 서는 문제가 남아 있어 기본은 꺼져 있습니다.
  크롬이 배경 작업을 재우기 때문인데, 저장된 위치에서 스스로 이어하도록 해 두었습니다.
  PC 앱과 안드로이드는 해당되지 않습니다.)
...
새 버전이 올라오면 **누르기를 기다리지 않고 바로 갈아 끼웁니다.**

확인은 한 시간마다, 그리고 팝업·앱을 열 때와 **자동화가 한 번 끝날 때마다** 합니다
(끝난 직후가 프로그램을 다시 켜기에 가장 안전한 때라, 미뤄 뒀던 설치도 이때 끝냅니다).
PC 설치판은 받자마자 스스로 다시 시작해 설치하고, 휴대용은 새 exe 를 옆에 받아 엽니다.
안드로이드는 APK 를 받아 설치 화면을 띄웁니다(안드로이드가 '설치' 한 번을 요구합니다).

크롬 확장만 한 단계가 남습니다 — 크롬은 개발자 모드로 넣은 확장의 파일을 스스로 갈아 끼우지 않습니다.
대신 **새 버전을 찾는 즉시 zip 을 알아서 받아 두고**, 폴더를 덮어쓰기만 하면
확장이 그것을 1분 안에 스스로 알아채고 다시 켜집니다(`chrome://extensions` 새로고침도 필요 없습니다).
...
| 📱 안드로이드 | 앱이 켜져 있는 동안 한 시간마다 새 APK 를 받아 **설치 화면을 띄웁니다** (자동화 중이면 끝난 직후) | '설치' 한 번 (안드로이드 규칙) |
| 🧩 크롬 확장 | 새 버전을 찾는 **즉시 zip 을 알아서 받아 둡니다**. 폴더를 덮어쓰면 1분 안에 스스로 다시 켜집니다 | 압축을 풀어 확장 폴더에 덮어쓰기 (개발자 모드 확장의 파일은 크롬이 스스로 못 바꿉니다) |
