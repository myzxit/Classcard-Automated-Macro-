...
const mock = readFileSync('/tmp/shots/mock_rsent.html','utf8');

const ctx = await chromium.launchPersistentContext('/tmp/shots/profileRSENT',{headless:false,
...
await popup.click('.mode-btn[data-id="recall_sentence"]');
await popup.waitForTimeout(150);
await popup.click('#btnRun');

await page.bringToFront();
await page.waitForTimeout(6000);
await page.screenshot({path:`${OUT}/rsent-solving.png`});
...
try {
  ...
  await page.screenshot({path:`${OUT}/rsent-done.png`});
} catch (e) {
  console.log('완주 못함. 현재 화면:',
    await page.evaluate(() => document.querySelectorAll('.talk-card:not(.hidden)').length + '장'));
  await page.screenshot({path:`${OUT}/rsent-timeout.png`});
}
await popup.bringToFront();
await popup.click('#tabLog');
await popup.waitForTimeout(400);
console.log('진단:', await page.evaluate(() => ({
  placed: (document.querySelector('.input-box')||{}).textContent,
  left: document.querySelectorAll('.btn-scramble.clickable:not(.clicked)').length,
  answers: (window.__cc_answers||[]).length })));
console.log('--- 로그 ---');
