...
const mock = readFileSync('/tmp/shots/mock_gclass_modal.html','utf8');
const ctx = await chromium.launchPersistentContext('/tmp/shots/profileGMODAL',{headless:false,
...
try {
  await page.waitForFunction(() => document.getElementById('stat').getAttribute('data-done') === '1',
    { timeout: 60000 });
  console.log('결과:', (await page.textContent('#stat')).replace('결과: ',''));
} catch (e) {
  console.log('결과: 확인 창을 누르지 못함(시간 초과)');
}
await page.screenshot({path:`${OUT}/gclass-modal.png`});
await ctx.close();
