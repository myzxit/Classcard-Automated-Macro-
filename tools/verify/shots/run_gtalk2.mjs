...
const mock = readFileSync('/tmp/shots/mock_gtalk2.html','utf8');

const ctx = await chromium.launchPersistentContext('/tmp/shots/profileGTALK2',{headless:false,
...
await page.screenshot({path:`${OUT}/gtalk2-solving.png`});
...
console.log('진단:', await page.evaluate(() => {
  ...
  return { card_idx, cards: cards.length, type: c && c.getAttribute('data-type'),
    orders: c ? [].map.call(c.querySelectorAll('.order-item'), e=>e.textContent+(e.classList.contains('selected')?'*':'')) : null,
    cls: c && c.className, tagged: document.querySelectorAll('[data-cc-order]').length };
}));
console.log('--- 로그 ---');
