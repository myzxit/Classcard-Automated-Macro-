...
const mock = readFileSync('/tmp/shots/mock_memorize.html','utf8');

const ctx = await chromium.launchPersistentContext('/tmp/shots/profilememorize',{headless:false,
...
await popup.click('.mode-btn[data-id="memorize"]');
await popup.waitForTimeout(150);
await popup.click('#btnRun');

await page.bringToFront();
await page.waitForTimeout(6000);
await page.screenshot({path:`${OUT}/memorize-solving.png`});
...
try {
  ...
  await page.screenshot({path:`${OUT}/memorize-done.png`});
} catch (e) {
  console.log('완주 못함. 현재 화면:',
    await page.evaluate(() => document.querySelectorAll('.talk-card:not(.hidden)').length + '장'));
  await page.screenshot({path:`${OUT}/memorize-timeout.png`});
}
await popup.bringToFront();
