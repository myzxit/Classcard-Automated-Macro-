...
const mock = readFileSync('/tmp/shots/mock_talk_type.html','utf8');

const ctx = await chromium.launchPersistentContext('/tmp/shots/profileTALKTYPE',{headless:false,
...
await page.screenshot({path:`${OUT}/talktype-solving.png`});
...
try {
  ...
  await page.screenshot({path:`${OUT}/talktype-done.png`});
} catch (e) {
  console.log('완주 못함. 현재 화면:',
    await page.evaluate(() => document.querySelectorAll('.talk-card:not(.hidden)').length + '장'));
  await page.screenshot({path:`${OUT}/talktype-timeout.png`});
}
await page.bringToFront();
console.log('진단:', await page.evaluate(() => ({
  boxes: document.querySelectorAll('[data-cc-tinput]').length,
  vals: [...document.querySelectorAll('[data-cc-tinput]')].map(e=>[e.getAttribute('data-cc-tinput'), e.getAttribute('data-cnt'), e.value, !!e.closest('.talk-card')]),
  hasArr: typeof arr_card,
  cards: document.querySelectorAll('.talk-card').length,
})));
await popup.bringToFront();
